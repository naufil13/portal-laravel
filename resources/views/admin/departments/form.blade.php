                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            <label for="user_type_id"
                                class="col-3 col-form-label text-right required">{{ __('Division') }}: <span
                                    class="text-danger">*</span></label>
                            <div class="col-6">
                                <select name="divisions_id" id="divisions_id" class="form-control m-select2">
                                    <option value="">Select Division</option>
                                    @php
                                        echo selectBox("SELECT id, division_name FROM divisions WHERE tenant_id='" . Auth::user()->userclient['id'] . "'", old('divisions_id', Crypto::decryptData($row->divisions_id, Crypto::getAwsEncryptionKey())));
                                    @endphp
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            <label for="module" class="col-3 col-form-label text-right required">Department Name: <span
                                    class="text-danger">*</span></label>
                            <div class="col-6">
                                <input name="department_name"
                                    value="{{ old('department_name', Crypto::decryptData($row->department_name, Crypto::getAwsEncryptionKey())) }}"
                                    class="form-control" type="text" placeholder="Enter Department Name">
                            </div>
                        </div>

                        <div class="form-group row">
                            <label for="module" class="col-3 col-form-label text-right required">Department UUID: <span
                                    class="text-danger">*</span></label>
                            <div class="col-6">
                                <input name="department_uuid"
                                    value="{{ old('department_uuid', Crypto::decryptData($row->department_uuid, Crypto::getAwsEncryptionKey())) }}"
                                    class="form-control" type="text" placeholder="Enter Department UUID">
                            </div>
                        </div>
    ...
    <script type="text/javascript">
        $(document).ready(function($) {
            ...
            $('#tenants_id').on('change', function(e) {
                var tenants_id = $(this).val();
                $.ajax({
                    type: "get",
                    url: `{{ admin_url('getDivisionsByClientId', true) }}/` + tenants_id,
                    success: function(res) {
                        if (res.length > 0) {
                            console.log(res);
                            $("#divisions_id").empty();
                            $.each(res, function(index, stateObj) {
                                $('#divisions_id').append('<option value="' + stateObj
                                    .id + '">' + stateObj.division_name +
                                    '</option>');
                            })
                        } else {
                            $("#divisions_id").empty();
