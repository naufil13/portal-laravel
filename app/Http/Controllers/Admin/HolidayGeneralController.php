...
class HolidayGeneralController extends Controller
{
    ...
    {
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'group' => 'holiday_generals.' . $this->id_key])->merge(request()->query())->toArray();
        ...
        //$SQL = "";
        $select = "holiday_generals.id
        , holiday_generals.holiday_description
        , DATE_FORMAT(holiday_generals.holiday_date, '%m-%d-%Y') AS Holiday";
        $where = $this->where;
        ...
        $paginate_OBJ
            ...
            ->transform(function ($item, $key) {
                return [
                    'id' => $item['id'],
                    'holiday_description' => Crypto::decryptData($item['holiday_description'], Crypto::getAwsEncryptionKey()),
                    'Holiday' => Crypto::decryptData($item['Holiday'], Crypto::getAwsEncryptionKey()),
                ];
            });
        ...
        if (request()->ajax()) {
            ...
            return view('admin.holiday_generals.grid', compact('paginate_OBJ', 'query'), ['_info' => $this->_info]);
        }
    }
    ...
    public function form()
    {
        ...
        // dd($clients->);
        return view('admin.holiday_generals.form', compact('row'));
    }
    ...
    public function store()
    {
        ...
        $validator = Validator::make(request()->all(), [
            ...
            'holiday_date' => "required|unique:{$this->module},holiday_date,{$id},{$this->id_key}",
        ]);
        ...
        $data['data']['holiday_date'] = request()->input('holiday_date');
    ...
    public function delete()
    {
        ...
        foreach ($datas as $data) {
            if (strpos($data->holidays_id, $id)) {
                $alert = ['class' => 'error', 'message' => "Cannot Delete, Sites Exist Against This Holidays !!"];
