...
class EventChargeController extends Controller
{
    ...
    {
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'group' => 'event_charges.' . $this->id_key])->merge(request()->query())->toArray();
        ...
        //$SQL = "";
        $select = "event_charges.id
        , event_registrations.event_name
        ...
        $SQL = $SQL->join('event_registrations', 'event_charges.event_registration_id', '=', 'event_registrations.id');
        ...
        $paginate_OBJ
            ...
            ->transform(function ($item, $key) {
                return [
                    'id' => $item['id'],
                    'event_name' => Crypto::decryptData($item['event_name'], Crypto::getAwsEncryptionKey()),
                    'charges' => Crypto::decryptData($item['charges'], Crypto::getAwsEncryptionKey()),
                ];
            });
        ...
        if (request()->ajax()) {
            ...
            return view('admin.event_charges.grid', compact('paginate_OBJ', 'query'), ['_info' => $this->_info]);
        }
    }
    ...
    public function form()
    {
        ...
        // dd($clients->);
        return view('admin.event_charges.form', compact('row'));
    }
    ...
    public function store()
    {
        ...
        $data['data']['event_registration_id'] = request()->input('event_registration_id');
