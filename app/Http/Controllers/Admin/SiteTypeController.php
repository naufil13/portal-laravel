...
class SiteTypeController extends Controller
{
    ...
    {
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'group' => 'site_types.' . $this->id_key])->merge(request()->query())->toArray();
        ...
        if (request()->ajax()) {
            ...
            return view('admin.site_types.grid', compact('paginate_OBJ', 'query'), ['_info' => $this->_info]);
        }
    }
    ...
    public function form()
    {
        ...
        // dd($clients->);
        return view('admin.site_types.form', compact('row'));
    }
    ...
    public function store()
    {
        ...
        $validator = Validator::make(request()->all(), [
            'type' => "required|unique:{$this->module},type,{$id},{$this->id_key}",
        ]);
    ...
    public function delete()
    {
        ...
        $check_site_existence = DB::table('sites')
            ->select('*')
            ->where('site_type_id', $id)
            ->first();

        if (isset($check_site_existence)) {
            $alert = ['class' => 'error', 'message' => "Cannot Delete, Sites Exist Against This Type !!"];
