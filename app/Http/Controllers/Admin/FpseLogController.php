...
class FpseLogController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method fpse_log index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 100, 'group' => 'fpse_logs.' . $this->id_key])->merge(request()->query())->toArray();

        $res = curl_request('https://fpse.lathransoft.com:8085/getActivityLog');
        $arr_body = json_decode($res);
        $res_body = collect($arr_body->data);

        $Response = $res_body->transform(function($transform){
            $user_name = DB::table('users')->select('users.email')->where('id',$transform->id)->first();
            $transform->email = Crypto::decryptData($user_name->email, Crypto::getAwsEncryptionKey());
            return $transform;
        });
        ...
        return view('admin.fpse_logs.grid  ', compact('Response'), ['_info' => $this->_info]);
        }
    }
    ...
    public function form()
    {
        ...
        return view('admin.fpse_logs.form', compact('row'));
    }
    ...
    public function view()
    {
        ...
        $res = curl_request("https://fpse.lathransoft.com:8085/getActivityLogById/$id");
        $arr_body = json_decode($res);
        $res_body = $arr_body->data;
        ...
        if (request()->ajax()) {
            ...
            return view('admin.fpse_logs.view', compact('row', 'config'), ['_info' => $this->_info]);
        } else {
