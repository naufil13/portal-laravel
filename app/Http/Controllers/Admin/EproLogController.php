...
class EproLogController extends Controller
{
    ...
    /**
     * *****************************************************************************************************************
     * @method epro_log index | Grid | listing
     * *****************************************************************************************************************
     */
    public function index()
    {
        ...
        /** -------- Pagination Config */
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 100, 'group' => 'epro_logs.' . $this->id_key])->merge(request()->query())->toArray();

        $res = curl_request('https://epro.lathransoft.com:8084/getLogs');
        $res_body = json_decode($res);

        /** -------- RESPONSE */
        if (!$res_body){
            return \Redirect::to(admin_url('', true))->with('error', 'Failed to Fetch Data!');
        }else{
            return view('admin.epro_logs.grid  ', compact('res_body'), ['_info' => $this->_info]);
        }
    }
    ...
    public function form()
    {
        ...
        return view('admin.epro_logs.form', compact('row'));
    }
    ...
    public function view()
    {
        ...
        $res = curl_request("https://epro.lathransoft.com:8084/getLogs/$id");
        $res_body = json_decode($res);
        ...
        if (request()->ajax()) {
            ...
            return view('admin.epro_logs.view', compact('row', 'config'), ['_info' => $this->_info]);
        } else {
