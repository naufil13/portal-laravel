...
class ApplicationController extends Controller
{
    ...
    {
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'group' => 'applications.' . $this->id_key])->merge(request()->query())->toArray();

        //$SQL = "";
        $select = "applications.id
        , applications.application_name
        , applications.application_code
        , DATE_FORMAT(applications.ga_release_date, '%m-%d-%Y') AS Date_Released";
        $where = $this->where;
        ...
        // $SQL = $SQL->leftJoin('application_client', 'applications.id', '=', 'application_client.application_id');
        ...
        if (request()->ajax()) {
            ...
            return view('admin.applications.grid', compact('paginate_OBJ', 'query'), ['_info' => $this->_info]);
        }
    }
    ...
    public function form()
    {
        ...
        $allw_roles = DB::table('application_roles')
            ->select('allowed_role_id')
            ->where('application_id', $id)
            ->get()->toArray();
        // dd($allw_roles);

        return view('admin.applications.form', compact('row', 'tokens', 'roles', 'allw_roles'));
    }
    ...
    public function store()
    {
        ...
        $validator = Validator::make(request()->all(), [
            ...
            'application_url' => "required|max:255",
            'application_description' => "required|max:255",
            'application_code' => "required|unique:{$this->module},application_code,{$id},{$this->id_key}|max:255",
            'application_status' => "required|max:255",
            'ga_version' => "required|max:255",
            'token_id' => "required|max:255",
            'platform' => "required|max:255",
            'ga_release_date' => "required|max:255",
            'technology_stack' => "required|max:255",
         ...
         * | File Upload
         *‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒*/
        $files = upload_files(['image', 'icon'], "assets/admin/media/applications");
        ...
        if ($row->save()) {
            ...
            if ($id == 0) {
                ...
                DB::table('application_roles')->where('application_id', '=', $id)->delete();
                foreach ($allowed_roles as $allowed_role) {
                    DB::table('application_roles')->insert([
                        'application_id' => $id,
                        'allowed_role_id' => $allowed_role,
