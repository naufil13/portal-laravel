...
class ApplicationRoleController extends Controller
{
    ...
    {
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'group' => 'application_roles.' . $this->id_key])->merge(request()->query())->toArray();

        //$SQL = "";
        $select = "application_roles.id
        ...
        if (request()->ajax()) {
            ...
            return view('admin.application_roles.grid', compact('paginate_OBJ', 'query'), ['_info' => $this->_info]);
        }
    }
    ...
    public function form()
    {
        ...
        // dd($clients->);
        return view('admin.application_roles.form', compact('row'));
    }
    ...
    public function store()
    {
        ...
        $validator = Validator::make(request()->all(), [
            ...
            'role' => "required|unique:{$this->module}",
        ]);
