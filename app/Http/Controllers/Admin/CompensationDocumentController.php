...
class CompensationDocumentController extends Controller
{
    ...
    public function index()
    {
        ...
        $user_actions = array_unique(explode('|', str_replace(array('update'), array('edit'), $row->user_actions)));
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'group' => 'compensation_documents.' . $this->id_key])->merge(request()->query())->toArray();
        ...
        $compensation_docs = DB::table('compensation_documents')
        ->select('compensation_documents.*')
        ->get();
        ...
        //$SQL = "";
        $select = "compensation_documents.id
        , compensation_documents.name
        , compensation_documents.date
        , compensation_documents.document
        , compensation_documents.completion
        , compensation_documents.exception
        , compensation_documents.check_no";
        $where = $this->where;
        ...
        if (request()->ajax()) {
            ...
            return view('admin.compensation_docs.grid', compact('paginate_OBJ', 'query', 'compensation_docs','user_actions'), ['_info' => $this->_info]);
        }
    }
    ...
    public function store()
    {
        ...
        $data['data']['name'] = request()->input('file_name');
        $data['data']['date'] = request()->input('date');
        ...
        $files = upload_files(['document'], "assets/admin/media/compansation_document");

        foreach ($files as $name => $file) {
            if ($file) {
                $data['data']['document'] = $file->getFilename();
            }
        }
    ...
    public function documentUpload()
    {
        ...
        if(request()->input('title')=="completion"){
            $files = upload_files(['completion'], "assets/admin/media/compansation_document");
        }
        else  if(request()->input('title')=="exception"){
            $files = upload_files(['exception'], "assets/admin/media/compansation_document");
        }
        else{
            $files = upload_files(['check_no'], "assets/admin/media/compansation_document");
        }
        ...
        foreach ($files as $name => $file) {
            if ($file) {
                if(request()->input('title')=="completion"){
                    $data['data']['completion'] = $file->getFilename();
                }
                else  if(request()->input('title')=="exception"){
                $data['data']['exception'] = $file->getFilename();
                }
                else{
                    $data['data']['check_no'] = $file->getFilename();
                }
            }
        }
            ...
            if ($row->save()) {
                    $id = $row->{$this->id};
                    set_notification('Document upload successfully!', 'success');
                    activity_log('Add', $this->table, $id, 0, null, $data['data']);
                    $__redirect = (!empty(getVar('__redirect')) ? getVar('__redirect') : admin_url("index/", true));
                    return redirect($__redirect)->with('primary', 'Document upload successfully!');
            }
    }
