...
class TenantController extends Controller
{
    ...
    {
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'tenants.' . $this->id_key])->merge(request()->query())->toArray();

        //$SQL = "";
        $select = "tenants.id
        , tenants.tenant_name
        , tenants.login_code
        , tenants.tenant_email
        , tenants.status";
        $where = $this->where;
        ...
        $paginate_OBJ
            ...
            ->transform(function ($item, $key) {
                return [
                    'id' => $item['id'],
                    'tenant_name' => Crypto::decryptData($item['tenant_name'], Crypto::getAwsEncryptionKey()),
                    'login_code' => $item['login_code'],
                    'tenant_email' => Crypto::decryptData($item['tenant_email'], Crypto::getAwsEncryptionKey()),
                    'status' => $item['status']
                ];
            });
        // dd($paginate_OBJ);
        /**‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒
         * | Encryption of Cipher Text Ends...
         *‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒*/

        if (request()->ajax()) {
            return $paginate_OBJ;
        } else {
            return view('admin.tenants.grid', compact('paginate_OBJ', 'query'), ['_info' => $this->_info]);
        }
    }
    ...
    public function form()
    {
        ...
        if ($id > 0) {
            ...
            $up_applications = DB::table('application_tenant')
                ->select(DB::raw('id, tenant_id, application_id'))
                ->where('tenant_id', '=', $id)
                ->get();
            if ($row->id <= 0) {
        ...
        return view('admin.tenants.form', compact('row', 'applications', 'up_applications'));
    }
    ...
    public function store()
    {
        ...
        $validator = Validator::make(request()->all(), [
            'tenant_name' => "required|unique:{$this->module},tenant_name,{$id},{$this->id_key}|max:255",
            'login_code' => "required|unique:{$this->module},login_code,{$id},{$this->id_key}|max:255",
            'contact_person_name' => 'required|max:255',
            'tenant_email' => 'required|email|max:255',
            'tenant_phone' => 'required|max:255',
            'tenant_address' => 'required|max:255',
            'country' => 'required|max:255',
            'state' => 'required|max:255',
            'city' => 'required|max:255',
            'zip_code' => 'required|numeric',
            'application_id' => 'required',
            'tenant_logo' => 'mimes:png,jpg,jpeg'
        ]);
        ...
        $files = upload_files(['tenant_logo'], "assets/admin/media/tenant_logo");
         ...
         * | File Upload
         *‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒*/
        $data['data']['login_code'] = request()->input('login_code');
        $data['data']['country'] = request()->input('country');
        $data['data']['state'] = request()->input('state');
        $data['data']['city'] = request()->input('city');
        ...
        if ($row->save()) {
            $tenant_apps = request()->input('application_id');
            if ($id == 0) {
                $id = $row->{$this->id_key};

                foreach ($tenant_apps as $tenant_app) {
                    DB::table('application_tenant')->insert([
                        ['tenant_id' => $id, 'application_id' => $tenant_app]
                    ]);
                }
                ...
                DB::table('application_tenant')->where('tenant_id', '=', $id)->delete();
    ...
    public function delete()
    {
        ...
        $alert = ['class' => 'primary', 'message' => "Record has been deleted!"];
