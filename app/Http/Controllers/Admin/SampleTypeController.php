...
class SampleTypeController extends Controller
{
    ...
    {
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'group' => 'sample_types.' . $this->id_key])->merge(request()->query())->toArray();
        ...
        , sample_types.name
        , sample_type_units.unit
        , sample_types.limit
        , sample_types.duration
        , sample_types.status";
        $where = $this->where;
        ...
        $SQL->join('sample_type_units', 'sample_types.sample_type_unit_id', '=', 'sample_type_units.id');
        ...
        if (request()->ajax()) {
            ...
            return view('admin.sample_types.grid', compact('paginate_OBJ', 'query'), ['_info' => $this->_info]);
        }
    }
    ...
    public function form()
    {
        ...
        $sample_types_rel = DB::table('sample_types_min_max_rates')
            ->select('*')
            ->where('sample_types_id', '=', $id)
            ->get();

        $sample_type_units = DB::table('sample_type_units')->get();

        return view('admin.sample_types.form', compact('row', 'sample_types_rel', 'sample_type_units'));
    }
    ...
    {
        $id = (int)request()->input($this->id_key);
        $rules = [
            'name' => "required",
            'measurement' => "required",
        ];
        $dataExist = $this->model->where('tenant_code', Auth::user()->userclient['login_code'])->where('name', request()->name)->where($this->id_key, '!=', $id)->count();

        if($id <= 0 || $dataExist > 0) {
            $rules['name'] .= "|unique:{$this->module},name,{$id},{$this->id_key}";
        }

        // return SampleType::where('tenant_code', Auth::user()->userclient['login_code'])->where('name', request()->name)->where('id', '!=', $id)->get();
        ...
        $data['data']['duration'] = request()->input('duration');
        $data['data']['name'] = request()->input('name');
        ...
        $data['data']['sample_type_unit_id'] = request()->input('measurement');
        $data['data']['no_compensation'] = request()->input('no_compensation');
        $data['data']['no_collection'] = request()->input('no_collection');

        $minimum = array_filter(request()->input('minimum'));
        $maximum = array_filter(request()->input('maximum'));
        $payment_rate = array_filter(request()->input('payment_rate'));
        ...
        if ($id > 0) {
            ...
            $check_type_existence = DB::table('trials_sample_types')
            ->select('*')
            ->where('sample_types_id', $id)
            ->first();

            if (isset($check_type_existence)) {
            ...
            if ($id == 0) {
                ...
                if (count($minimum) > 0) {
                    for ($i = 0; $i < count($minimum); $i++) {
                        $data_sample_types_min_max_rates = [
                            'sample_types_id' => $id,
                            'minimum' => $minimum[$i],
                            'maximum' => $maximum[$i],
                            'payment_rate' => $payment_rate[$i],
                            'tenant_code' => Auth::user()->userclient['login_code'],
                            'tenant_id' => Auth::user()->userclient['id'],
                            'created_by' => Auth::user()->email,
                        ];
                        $insertedId = DB::table('sample_types_min_max_rates')->insertGetId($data_sample_types_min_max_rates);
                        activity_log('Add', 'sample_types_min_max_rates', $insertedId, 0, null, $data_sample_types_min_max_rates);
                    }
                }
                ...
                if (count($minimum) > 0) {
                    DB::table('sample_types_min_max_rates')->where('sample_types_id', '=', $id)->delete();

                    for ($i = 0; $i < count($minimum); $i++) {
                        DB::table('sample_types_min_max_rates')->insert([
                            [
                                'sample_types_id' => $id,
                                'minimum' => $minimum[$i],
                                'maximum' => $maximum[$i],
    ...
    {
        $unit = request()->unit;
        DB::table('sample_type_units')->insert(['unit' => $unit]);
        // return \Redirect::to(admin_url('form', true));
    ...
    public function delete()
    {
        ...
        $get_type = DB::table('sample_types')
            ->select('name')
            ->where('id', $id)
            ->first();

        $datas = DB::table('sites')
            ->select('sample_type_collection_capacity')
            ->get();
        ...
        if (isset($check_type_existence)) {
            $alert = ['class' => 'error', 'message' => "Cannot Delete, Trail Exist Against This Sample Type !!"];
            return \Redirect::to(admin_url('index', true))->with($alert['class'], $alert['message']);
        }

        foreach ($datas as $data) {
            if (strpos($data->sample_type_collection_capacity, strtolower($get_type->name))) {
                $alert = ['class' => 'error', 'message' => "Cannot Delete, Sites Exist Against This Sample Type !!"];
                return \Redirect::to(admin_url('index', true))->with($alert['class'], $alert['message']);
            }
        }
