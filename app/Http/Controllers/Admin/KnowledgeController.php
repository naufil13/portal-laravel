...
class KnowledgeController extends Controller
{
    ...
    {
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'group' => 'knowledges.' . $this->id_key])->merge(request()->query())->toArray();
        ...
        , knowledges.document_name
        , knowledges.version
        , DATE_FORMAT(knowledges.published_date, '%m-%d-%Y') AS Published_Date
        , knowledges.published_by
        , knowledges.filename AS document";
        $where = $this->where;
        ...
        $SQL = $SQL->join('applications', 'knowledges.application_name', '=', 'applications.id');
        // dd($SQL);
        ...
        $paginate_OBJ
            ...
            ->transform(function ($item, $key) {
                return [
                    ...
                    'document_name' => Crypto::decryptData($item['document_name'], Crypto::getAwsEncryptionKey()),
                    'version' => Crypto::decryptData($item['version'], Crypto::getAwsEncryptionKey()),
        ...
        if (request()->ajax()) {
            ...
            return view('admin.knowledges.grid', compact('paginate_OBJ', 'query'), ['_info' => $this->_info]);
        }
    }
    ...
    public function form()
    {
        ...
        // dd($row);
        return view('admin.knowledges.form', compact('row'));
    }
    ...
    public function store()
    {
        ...
        $data['data']['document_name'] = request()->input('document_name');
        $data['data']['version'] = request()->input('version');
        $data['data']['published_date'] = request()->input('published_date');
        $data['data']['application_name'] = request()->input('application_name');
        $data['data']['published_by'] = Crypto::decryptData(Auth::user()->first_name, Crypto::getAwsEncryptionKey()) . ', ' . Crypto::decryptData(Auth::user()->last_name, Crypto::getAwsEncryptionKey());
         ...
         * | File Upload
         *‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒*/
        $files = upload_files(['filename'], "assets/admin/media/knowledge_hub");
    ...
    public function delete()
    {
        ...
        $unlink = ['filename' => asset_dir('media/knowledge_hub/', true)];
