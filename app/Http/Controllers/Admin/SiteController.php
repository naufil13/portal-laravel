...
class SiteController extends Controller
{
    ...
    {
        // dd('test');
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'group' => 'sites.' . $this->id_key])->merge(request()->query())->toArray();
        ...
        $paginate_OBJ
            ...
            ->transform(function ($item, $key) {
                return [
                    'id' => $item['id'],
                    'site_name' => Crypto::decryptData($item['site_name'], Crypto::getAwsEncryptionKey()),
                    'street_address' => Crypto::decryptData($item['street_address'], Crypto::getAwsEncryptionKey()),
                ];
            });
        ...
        if (request()->ajax()) {
            ...
            return view('admin.sites.grid', compact('paginate_OBJ', 'query'), ['_info' => $this->_info]);
        }
    }
    ...
    public function form()
    {
        ...
        if ($user_type != 1) {
            $sample_types = DB::table('sample_types')
                ->select('id', 'name')
                ->where('tenant_code', Auth::user()->login_code)
                ->get();
        } else {
            $sample_types = DB::table('sample_types')
                ->select('id', 'name')
        ...
        foreach ($sample_types as $key => $value) {
            $sample_types[$key]->name =
                Crypto::decryptData($value->name, Crypto::getAwsEncryptionKey());
        }

        $sites_studies =
            DB::table('sites_clinical_studies')
            ->select('id', 'site_id', 'clinical_trial_id')
            ->where('site_id', '=', $id)
            ->get();
        ...
        return view('admin.sites.form', compact('row', 'sample_types', 'weekdays', 'sites_studies', 'tenants'));
    }
    ...
    public function store()
    {
        ...
        $validator = Validator::make(request()->all(), [
            'tenants_id' => "required",
            'site_code' => "required|unique:{$this->module},site_code,{$id},{$this->id_key}",
            'site_name' => "required|unique:{$this->module},site_name,{$id},{$this->id_key}",
            'site_contact_person_name' => "required",
            ...
            'office_hours_from' => "required",
            'office_hours_to' => "required",
        ]);
        ...
        $data['data']['country'] = request()->input('country');
        $data['data']['city_id'] = request()->input('city_id');
        $data['data']['state_id'] = request()->input('state_id');
        $data['data']['site_type_id'] = request()->input('site_type_id');
        $data['data']['timezone_id'] = request()->input('timezone_id');
        $data['data']['tenant_code'] = $login_code[0]->login_code;
        $data['data']['tenant_id'] = $tenants_id;
        ...
        foreach ($sample_types as $sample_type) {
            $arr[] =
                [strtolower(str_replace(' ', '_', $sample_type->name)) => request()->input(strtolower(str_replace(' ', '_', $sample_type->name)))];
        }
        // dd($arr);
        $data['data']['sample_type_collection_capacity'] = json_encode($arr);
        $data['data']['holidays_id'] = json_encode(request()->input('holidays_id'));
        $data['data']['working_days'] = json_encode(request()->input('working_days'));
        $data['data']['office_hours_from'] = json_encode(array_values(array_filter(request()->input('office_hours_from'))));
        $data['data']['office_hours_to'] =
            json_encode(array_values(array_filter(request()->input('office_hours_to'))));
        // dd($data['data']);
        /**‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒
         * | File Upload
         *‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒‒*/
    ...
    public function delete()
    {
        ...
        $check_trial_existence = DB::table('sites_clinical_studies')
            ->select('*')
            ->where('site_id', $id)
            ->first();

        if (isset($check_trial_existence)) {
            $alert = ['class' => 'error', 'message' => "Cannot Delete, Trial Exist Against This Site !!"];
