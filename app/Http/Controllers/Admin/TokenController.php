...
class TokenController extends Controller
{
    ...
    {
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'group' => 'tokens.' . $this->id_key])->merge(request()->query())->toArray();
        ...
        , tokens.name
        , tokens.status
        , DATE_FORMAT(tokens.created_at, '%m-%d-%Y') AS Created_At";
        $where = $this->where;
        ...
        if (request()->ajax()) {
            ...
            return view('admin.tokens.grid', compact('paginate_OBJ', 'query', 'tokens'), ['_info' => $this->_info]);
        }
    }
    ...
    public function delete()
    {
        ...
        $check_token_existence = DB::table('trials_token')
            ->select('*')
            ->where('token_id', $id)
            ->first();

        if (isset($check_token_existence)) {
            $alert = ['class' => 'error', 'message' => "Cannot Delete, Trail Exist Against This Token !!"];
