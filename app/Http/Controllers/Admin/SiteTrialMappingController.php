...
class SiteTrialMappingController extends Controller
{
    ...
    {
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'limit' => 25, 'group' => 'site_trial_mappings.' . $this->id_key])->merge(request()->query())->toArray();

        //$SQL = "";
        $select = "site_trial_mappings.id
        , sites.site_name
        , site_trial_mappings.site_id";
        $where = $this->where;
        ...
        $SQL = $SQL->join('sites', 'sites.id', '=', 'site_trial_mappings.site_id');
        ...
        if (request()->ajax()) {
            ...
            return view('admin.site_trial_mappings.grid', compact('paginate_OBJ', 'query'), ['_info' => $this->_info]);
        }
    }
    ...
    public function form()
    {
        ...
        // dd($clients->);
        return view('admin.site_trial_mappings.form', compact('row', 'tenants'));
    }

    public function getSitesByTenantId(int $tenant_id)
    {
    ...
    public function getTrialsByTenantId(int $tenant_id)
    {
    ...
    public function store()
    {
        ...
        $login_code = DB::table('tenants')
            ->select('login_code')
            ->where('id', '=', request()->input('tenant_id'))
            ->get();

        $data['data']['created_by'] = Auth::user()->email;
        $data['data']['tenant_code'] = $login_code[0]->login_code;

        $data['data']['trial_id'] = json_encode(request()->input('trial_id'));
        // dd($data['data']);
        if ($id > 0) {
            $row = $this->model->find($id);
