...
class DepartmentController extends Controller
{
    ...
    {
        $config = collect(['sort' => $this->id_key, 'dir' => 'desc', 'group' => 'departments.' . $this->id_key])->merge(request()->query())->toArray();
        ...
        $SQL = $SQL->join('cities', 'departments.city_id', '=', 'cities.id');
        $SQL = $SQL->join('states', 'departments.state_id', '=', 'states.id');
        $SQL = $SQL->join('countries', 'departments.country_id', '=', 'countries.id');
        ...
        $paginate_OBJ
            ...
            ->transform(function ($item, $key) {
                return [
                    'id' => $item['id'],
                    'department_name' => Crypto::decryptData($item['department_name'], Crypto::getAwsEncryptionKey()),
                    'department_uuid' => Crypto::decryptData($item['department_uuid'], Crypto::getAwsEncryptionKey()),
        ...
        if (request()->ajax()) {
            ...
            return view('admin.departments.grid', compact('paginate_OBJ', 'query'), ['_info' => $this->_info]);
        }
    }

    public function getDivisionsByClientId(int $tenants_id)
    {
    ...
    public function form()
    {
        ...
        return view('admin.departments.form', compact('row', 'tenants'));
    }
    ...
    public function store()
    {
        ...
        $validator = Validator::make(request()->all(), [
            ...
            'department_uuid' => "required|max:100",
        ...
        $data['data']['divisions_id'] = request()->input('divisions_id');
